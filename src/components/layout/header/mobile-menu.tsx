...
export const MobileMenu = ({ items, isOpen, onClose }: MobileMenuProps) => {
  ...
  const renderMenuItem = (item: MenuItem, level = 0, parentPath = "") => {
    ...
    return (
      <li
        key={fullPath}
        className={`tw-relative  tw-w-full ${isSubMenuOpen ? "open" : ""}`}
      >
        <div
          className={cn(
            "tw-flex tw-items-center tw-justify-between tw-w-full",
            level > 0 && "tw-border-l tw-border-gray-200 tw-ml-2"
          )}
        >
          {item.external ? (
            <a
            ...
            <>
              ...
              {hasSubMenu && (
                ...
                >
                  <ChevronDown
                    className={cn(
                      "tw-h-8 tw-w-8 tw-text-primary-500 tw-transition-transform",
                      isSubMenuOpen && "tw-transform tw-rotate-180"
                    )}
                  />
                </button>
              )}
        ...
        {hasSubMenu && (
          <ul
            className={cn(
              ...
              isSubMenuOpen
                ? "tw-max-h-[5000px] tw-opacity-100 tw-visible"
                : "tw-max-h-0 tw-opacity-0 tw-invisible tw-pointer-events-none"
            )}
          >
  ...
  return (
    ...
    >
      <div
        className="menu-close tw-absolute tw-top-0 tw-right-0 btn-primary tw-w-10 tw-h-10 tw-flex tw-justify-center tw-items-center tw-text-white tw-cursor-pointer"
        onClick={onClose}
      >
          ...
          <ul className="social-media !tw-m-0 !tw-p-0">
            ...
            <li>
              <a
                target="_blank"
                rel="noreferrer"
                href={SocialAccountLinks.GOOGLE}
                className=" btn-primary"
                aria-label="Google"
              >
            ...
            <li>
              <a
                target="_blank"
                rel="noreferrer"
                href={SocialAccountLinks.TWITTER}
                className=" btn-primary"
                aria-label="Twitter"
              >
